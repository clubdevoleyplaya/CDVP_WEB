...
| A01 | ✅ (corregido 2026-08-13) | Ver `CDVP_Api/docs/seguridad/auditoria_2026-08-13.md` — el fix de RLS sobre `profiles.role` ya está aplicado al proyecto real, así que `proxy.ts` vuelve a ser confiable en su chequeo de `role` (nadie puede auto-asignárselo). | — | Cerrado. |
...
**#1/#2/#4 resueltos (2026-08-13):** `spec_02_privacidad-datos.md`, página `/privacidad` publicada y linkeada desde el footer — borrador, pendiente de revisión legal antes de un lanzamiento formal. **#5 parcialmente resuelto:** la página deja un canal para pedir la baja, pero es manual (mail placeholder sin completar todavía, no hay endpoint de autoservicio). **Los puntos #8/#9 son válidos para cuando se construya el checkout real** (`pagos-mercadopago`/`checkout-ui`, hoy no implementados) — dejar anotado en `TODO.md` para no repetir el error al construirlos.
