...
## Casos
- Testimonios en video de Campus (carpeta Drive `1KIWqYKGp5NA4Ac_LCGSSZlX5K9EWVxxc`, hoy 11 archivos sin curar) → se muestran mezclados sin restricción, es el volumen más alto y es contenido general del club.
- Testimonios en video de Campamento de entrenamiento (2 clips, carpeta Drive `1lqudM86YcnfTVBqufBGLcbPC_Xgf03fx`) → se muestran solo dentro de la sección propia de Campamento, nunca mezclados con los de Campus.
- Sección propia de Campamento → no existe todavía en el sitio (hoy `testimonials.tsx` solo tiene un bloque genérico de 3 tarjetas). Pendiente definir con el usuario si es un bloque separado dentro del mismo landing o una ruta propia, antes de implementar este caso.
